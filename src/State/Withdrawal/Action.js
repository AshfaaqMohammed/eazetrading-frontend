...
export const proceedWithdrawal = (jwt, id, accept) => async (dispatch) => {
    ...
    try {
        const { data } = await api.patch(`/api/admin/withdrawal/${id}/proceed/${accept}`, null, {
            headers: {
                Authorization: `Bearer ${jwt}`
            }
        });
...
export const getAllWithdrawalRequest = (jwt) => async (dispatch) => {
    ...
    try {
        ...
        console.log("all withdrawal requests - ", data);
        dispatch({ type: GET_WITHDRAWAL_REQUEST_SUCCESS, payload: data });
    } catch (error) {
